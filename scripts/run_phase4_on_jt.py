"""Run Phase 4 (Robustness Battery + D3) on the JT 1993 spec.

Depends on the cached A1 + A2 + A3 outputs. Runs every stress family,
emits a `RobustnessScorecard`, then asks D3 for a structured judgment.

Total wall time on first run: ~15-20 min for the engine reruns (each is
~30-60s). Cached after that — re-runs are seconds.
"""
...
def main() -> None:
    ...
    section("RUNNING ROBUSTNESS BATTERY (~15-20 min on first run, cached after)")
    ...
    print(f"  capacity_estimate:  "
          f"${scorecard.capacity_estimate_usd:,.0f}"
          if scorecard.capacity_estimate_usd else "  capacity_estimate:  n/a")
    print()
    ...
    section("D3 — Robustness Adversary (Opus)")
    print("Running D3 (cached)...", flush=True)
    judgment = judge(
        scorecard=scorecard,
        baseline=baseline,
    ...
    print(f"  capacity_estimate_usd: "
          f"${judgment.capacity_estimate_usd:,.0f}"
          if judgment.capacity_estimate_usd else "  capacity_estimate_usd: n/a")
