...
src/pdf/            pdfplumber parser (with pypdfium2 fallback for glyph-spaced OCR) + deterministic fuzzy quote verifier
...
scripts/            phase1_gate.py, run_jt_end_to_end.py, run_a1_on_jt.py,
                    ...
                    run_all_pipelines.py (audit harness — invokes
                    app.main._run_pipeline on every PDF in data/papers and
                    writes the resulting bundle to outputs/audit/<stem>.json)
...
- Single-page SPA, no build step, no Tailwind CDN — hand-rolled CSS matching the "Bridgewater Intelligence Briefing" tokens (deep-obsidian background `#0c0e12`, brick-red `#8b231c` + muted-gold `#b89762` accents, hairline `#2d333b` borders, sharp 0–2 px corners, Georgia serif for headers / Inter sans-serif for tabular data, no glow / gradient / rounded-pill effects). All snake_case schema field names route through `terminologyMap` + `term()` in `app.js` before they hit the DOM — never inject raw `lookback_months` / `monthly_return` strings into a renderer; add a mapping there instead. The Bridgewater wordmark sits inside the topnav `.brand` element (`<img class="brand-logo">` next to the title text) as a discrete attribution badge on a small light backplate — don't enlarge it, and don't add a second logo elsewhere on the page. (`.bw-watermark` is kept in CSS as a dormant rule in case the wordmark is re-attached to the bottom-right corner later.) The CIO Summary header on the verdict strip is injected via `.vs-paper::before` content; don't duplicate it in markup.
...
- **`SignalKind` is a four-state literal; three are natively implemented (Phase E).** `SignalSpec.kind ∈ {past_return, variance_ratio, fundamental_ratio, custom}`. `src/engine/signals.py::compute_signal` natively implements `past_return`, `variance_ratio`, and (Phase E) `fundamental_ratio` via the `FUNDAMENTAL_RATIO_REGISTRY` (gross_profitability, book_to_market, earnings_yield, asset_growth, accruals). Only `custom` triggers proxy substitution. Every engine endpoint goes through `prepare_spec_for_engine(spec)` in `app/main.py` (Phase F bundling) which runs the entire prep chain — store dispatch, window clip, kind fallback, lookback clamp, weighting fallback, portfolio coercion — and returns a typed `PreparedSpec` containing the substituted spec plus a tuple of `SpecAdaptation` records (one per substitution). The frontend's `detectProxyMode` reads `BacktestResult.spec_adaptations` directly instead of substring-matching `data_quality_flags`. The `variance_ratio` implementation computes `Var(rolling_12m_return) / (12 × Var(monthly_return))` per ticker over a 60-month default window using overlapping 12-month returns. `fundamental_ratio` reads `store.get_fundamentals(ticker, as_of_date)` via the `FundamentalDataSource` Protocol — `PointInTimeDataStore` satisfies it today; a future Bridgewater-internal data source plugs in by satisfying the same Protocol. If you add a new endpoint that touches the engine, call `prepare_spec_for_engine` once and you get the whole prep chain — no per-endpoint reimplementation.
...
- **`PROXY ONLY` and `NO PAPER TARGET` verdict tiers (Phase F: typed dispatch).** When `prepare_spec_for_engine` emits a `SpecAdaptation(kind='signal_kind_proxy_substitution')` (engine ran a 12-month past-return proxy) or `headline_claim` is null / non-comparable (Sharpe-Δ, VR statistic, statistical-test variant), the alpha number is NOT a verdict on the paper. `detectProxyMode(bt)` in `app.js` now checks `bt.spec_adaptations.some(a => a.kind === 'signal_kind_proxy_substitution')` instead of substring-matching `data_quality_flags`. The verdict-strip's `PROXY ONLY` / `NO PAPER TARGET` short-circuits dispatch on the typed `headline_claim.kind` discriminator. If you add a new dashboard surface that quotes `mean_return` / `implementable_alpha`, gate it on `detectProxyMode` + `detectNoTarget` or it will silently mislabel proxy runs as paper failures.
- **Glyph-spaced PDFs ("S e c o n d" instead of "Second") slip past pdfplumber and pypdfium2 fixes them.** Lo-MacKinlay 1988's OCR layer uses character-level advance widths that fool pdfplumber's word-grouping (default `x_tolerance=3`) into emitting one token per character, with lines occasionally rebuilt bottom-up. The fix lives in `src/pdf/parser.py::parse_pdf`: after pdfplumber extracts each page, `_looks_glyph_spaced(text)` flags pages where ≥40% of whitespace-split tokens are length 1 (and there are ≥50 tokens), and the parser re-extracts that page via `pypdfium2`. Tables still come from pdfplumber. If you replace pdfplumber wholesale, keep the heuristic — other scanned/OCR'd papers will trip it. Also: re-OCR'ing such a PDF with `ocrmypdf --force-ocr` does NOT fix it; the glyph metadata survives because it's encoded in the font's advance-width table, not the text stream. The parser-level fallback is the right layer.
- **HeadlineClaim is a discriminated union over six claim shapes (Phase B).** `src/specs/paper_metrics.py` exposes `HeadlineClaim = Annotated[Union[MonthlyLongShortReturn, NestedConditionalSortReturn, SharpeRatioDifference, VarianceRatioStatistic, RegressionAlpha, StatisticalTestClaim], Field(discriminator='kind')]`. A1's prompt (`methodology_extractor.md` rule 8) dispatches the paper into one of these six variants. The frontend's `detectStatisticalOnlyPaper` and `detectAqrStreaksPaper` (kept as helper names; their bodies now read `headline_claim.kind`) return true for `variance_ratio_statistic / statistical_test` and `sharpe_ratio_difference` respectively. D2 dispatch on whether the variant is comparable to `BacktestResult.mean_return` — see `_headline_claim_comparable_to_mean_return` in `app/main.py`. The deleted `_known_headline_claim` / `_known_paper_force_no_claim` / `_apply_known_headline_claim` / `_apply_known_verification_report` registries (and the JS-side `PAPER_CLAIM_FALLBACKS`, `MOMENTUM_PAPER_IDS`, `NO_TARGET_KEYWORDS` matchers) are gone; their work is done by claim-variant dispatch.
- **Long-short bucket convention is enforced by the spec validator (Phase D).** `PortfolioSpec._check_buckets` now requires `long_bucket=n_buckets, short_bucket=1` for any `long_short=True` spec. The sign of the strategy is carried entirely by `signal.direction` ('long_high' for momentum, 'long_low' for reversal; `compute_signal` negates the score for 'long_low' so bucket N always picks the paper's chosen long leg). Earlier the CHST 2017 paper emitted `(direction='long_low', long_bucket=1, short_bucket=5)` — double-encoded — and the engine cancelled the two flips and traded momentum; `extraction_verifier.PAPER_ID_OVERRIDES["cheng_hameed_subrahmanyam_titman_2017"]` patched it for that paper. Validator now rejects the pattern unconditionally; the patch is deleted. Long-only specs (`long_short=False`) are unaffected — `long_bucket` is free.
- **PaperExtractionOverride for unrecoverable PDFs (Phase F).** `src/specs/extraction_overrides.py` carries a typed override model bound to a SHA-256 hash of a verbatim PDF span. Override files live under `data/extraction_overrides/*.yaml`; `extract_and_verify` loads them and applies any whose checksum matches the parsed PDF text. If the PDF is replaced and the evidence span no longer matches, the override silently disables — so curated values can't leak forward onto a different document. This is the structural replacement for the deleted `_known_*` registries. The two papers in the active corpus that need overrides today (LM-1988 byte-scrambled, AMP-2013 mirrored Table I) require human curation: read a verbatim quote from a non-scrambled span, compute the checksum via `src.specs.compute_quote_checksum`, paste the value + curated `headline_claim` into a YAML file. Until populated, those papers' headline_claims will be whatever A1 emits — likely None or an `AmbiguityFlag` (which is honest).
- **Two narrow paper-id overrides remain, documented.** `src/agents/extraction/extraction_verifier.py::PAPER_ID_OVERRIDES["jegadeesh_titman_1993"]["universe.min_price"] = 10.0` (paper is silent on price filter; $10 matches the published replication community default) and `app/main.py::ENGINE_WINDOW_OVERRIDES["jegadeesh_titman_1993"] = (2007-01-01, 2026-04-02)` (paper's 1965-1989 window predates the defeatbeta data panel by ~30 years; the OOS window captures the post-2009 momentum revival). Both are config-shaped knobs, not unbounded runtime branches. If you add a new paper, do NOT add another paper-id entry here — use PaperExtractionOverride (quote-checksum-bound) or extend A1's AmbiguityFlag defaults instead.
...
- **A1's `headline_claim` is REQUIRED for empirical papers.** The A1 prompt enforces it (rule 8) — empirical papers without a single designated headline must populate the field with an inferred value AND raise a high-severity `AmbiguityFlag` with `parameter="headline_claim"`. `headline_claim = null` is allowed only for purely theoretical papers. Without a claim, D2 has nothing to compare against and the demo's "PAPER CLAIMED" column is blank. If you edit the A1 prompt or `HeadlineClaim` schema, also bust the disk cache for any paper whose cached extraction predates the change (cached pickles validate against `Optional[HeadlineClaim]` and silently keep `null`).
- **D3 needs a deterministic fallback when the LLM is unavailable.** `src/agents/validation/robustness_adversary.py::judge` wraps `call_claude` in `try/except` and returns `_fallback_judgment(scorecard, baseline)` on any exception. The fallback emits `confidence="low"`, `signal_type="not_evaluated"`, `gap_attribution="unexplained"`, `summary="D3 LLM call failed; deterministic fallback values used."`, and copies fragility signals straight from the scorecard. This mirrors D2's `_fallback_diagnosis` and keeps the verdict strip renderable when 529s hit. If you change `judge`'s signature, keep the fallback in lockstep.
