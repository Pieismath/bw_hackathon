...
- **`SignalKind` is a tri-state but the engine only implements one.** `SignalSpec.kind ∈ {past_return, fundamental_ratio, custom}` but `src/engine/signals.py::compute_signal` raises `NotImplementedError` for anything other than `past_return`. Every endpoint that calls the engine — `/api/backtest`, `/api/robustness`, `/api/diagnose`, and the long-running `/api/pipeline/start` — MUST pass the spec through `_engine_kind_fallback` (and `_clip_spec_to_data_window`) in `app/main.py` before handing it to `run_backtest` / `run_battery` / `diagnose`. The fallback substitutes `past_return` (defaulting `lookback_months=12` when the paper didn't carry a months window — AQR-style daily-streak signals, transformer outputs) and emits a `data_quality_flag` so the UI banner makes the proxy honest. If you add a new endpoint that touches the engine, wire both substitutions in or the next custom-kind paper crashes the tab.
