...
- **AQR streaks reports Sharpe-ratio differences, not a monthly L/S return — and the engine's "individual-stock VR" warning leaks across papers.** AQR 2024 "Hidden Value of Streaky Returns" sorts ~153 JKP factors on a variance-ratio measure of streakiness and reports Sharpe-ratio gaps between top/bottom VR terciles (t=2.67 annual, t=2.78 annualized monthly, footnote 9). It is `signal.kind=variance_ratio` so `detectStatisticalOnlyPaper(bundle)` returns true, but the generic "VR(k) testing for mean reversion vs random walk" banner is wrong for this paper. `detectAqrStreaksPaper(bundle)` in `app.js` (matches paper_id substrings `aqr_2024` / `streaky_returns` / `hidden_value_of_streaky`) flips the Backtest banner / verdict-strip pill / claim column to a Sharpe-ratio-paper narrative and surfaces the t=2.67/2.78 numbers. Symmetrically, `src/engine/backtest.py` originally emitted "AQR sorts JKP factors not individuals" any time `variance_ratio + individual-stock universe` co-occurred — wrong for Lo-MacKinlay / Poterba-Summers, where stock-level VR is exactly what the paper concept-implies. The flag is now gated on `spec.paper_title` containing "streaky"/"streak"/"AQR + hidden", and `_rewrite_aqr_flag_for_non_aqr_paper(result, paper_id)` in `app/main.py` busts the misleading flag from any cached `BacktestResult` (the disk-cached result predates the gate) and substitutes a "VR strategy ran on individual stocks — concept-implied portfolio" note. If you add another VR paper, decide which of the three flag families it belongs to (AQR-streaks, stat-test on stocks, stat-test on KF-factors) and route it through the right branch.
- **Known-paper headline-claim registry covers Cheng-Hameed-Subrahmanyam-Titman 2017 AND Asness-Moskowitz-Pedersen 2013.** AMP 2013 ("Value and Momentum Everywhere") stores Table I in mirrored character order so A1 can't recover the verbatim cell — `_known_headline_claim` in `app/main.py` hardcodes the US-stock momentum P3-P1 row (annualized 5.4%/yr ⇒ 0.0045/mo, t=2.08, Jan 1972 – Jul 2011) so D2/D3/verdict-strip have a real comparison target. CHST 2017 was already in the registry (1.683%/mo, t=7.80, 3M-loser-conditioned 1M reversal). When adding a new paper to the registry, also re-run the pipeline once via `scripts/run_all_pipelines.py` so the disk-cached D2 LLM call gets warmed against the new claim text — D2's cache key includes the claim, so a stale cache from the pre-override run still produces a wrong diagnosis until D2 fires fresh.
