...
Every endpoint that drives the engine (`/api/backtest`, `/api/robustness`, `/api/diagnose`, plus the orchestrated `/api/pipeline/start`) runs four pre-engine substitutions on the incoming spec: `_build_store_for_spec` selects the right data source based on `spec.universe.name`, `_prepare_spec_window` clips out-of-range date windows, `_engine_kind_fallback` short-circuits for the natively-supported signal kinds (`past_return`, `variance_ratio`) and substitutes a 12-month-momentum proxy for the unsupported ones (`fundamental_ratio`, `custom`), and `_engine_lookback_clamp` shrinks `signal.lookback_months` if it would consume the entire data panel and leave fewer than 24 signal observations (e.g. AQR streaks' 360-month lookback against a defeatbeta panel that only spans 30 years would otherwise produce zero formations and crash the engine). Two universes are wired today: `defeatbeta_all_equities` (US stocks, post-1994, survivorship-biased Yahoo source) and `ken_french_factors` (Ken French 6-factor zoo — Mkt-RF, SMB, HML, RMW, CMA, Mom — going back to 1973 with no survivorship bias). The KF factor universe is the right cross-section for AQR-style papers that sort factor portfolios rather than stocks; six factors is fewer than AQR's 153 JKP factors but is concept-faithful and produces real numbers from real data. The `variance_ratio` kind computes the AQR streakiness statistic `Var(rolling_12m_return) / (12 × Var(monthly_return))` per name over a 60-month default window. Substitutions emit `data_quality_flag` entries on the `BacktestResult`, surfaced in the UI as a yellow banner so the swap stays honest.
...
When the engine substituted `signal.kind` (`custom`/`fundamental_ratio` papers run as a 12-month-momentum proxy) OR when no headline claim exists (A1 missed it and no manual override was supplied), the verdict strip stops claiming "implementable alpha vs paper" and switches to two structural-validity tiers: `PROXY ONLY` (engine ran a substitute strategy, every number is a verdict on the proxy not the paper) and `NO PAPER TARGET` (engine ran the paper's strategy but there's no claim to compare against, so the alpha number is vs zero). The Backtest tab paints a fail-severity banner at the top in proxy mode listing every metric on the page that's a proxy result. The Overview tab's Run config panel includes three optional headline-override inputs (`monthly return %/mo`, `t-stat`, `window`) so users can manually supply the paper's number when A1 fails to extract it — common for papers reporting Sharpe ratios or regression alphas. **Statistical-test papers** (Lo-MacKinlay 1988, Poterba-Summers 1988, and similar variance-ratio / autocorrelation-test papers) genuinely don't have a tradeable monthly L/S claim — they report VR(k) statistics testing the random-walk null. `detectStatisticalOnlyPaper(bundle)` matches `signal.kind === 'variance_ratio'` + null `paper_claim` and rewrites the banner / verdict-strip subline / Why-popover summary to a paper-class-specific narrative ("paper has no tradeable L/S claim; the implementable alpha is the implicit contrarian/momentum strategy's return vs zero"), so the user isn't told to enter an override that doesn't exist in the paper. **AQR 2024 "Hidden Value of Streaky Returns"** also reports `signal.kind=variance_ratio` but isn't a random-walk-null test — it sorts ~153 JKP factors on streakiness and reports Sharpe-ratio differences (t=2.67 annual / t=2.78 annualized monthly, Exhibit 7 / footnote 9). `detectAqrStreaksPaper(bundle)` keys off the `paper_id` substring (`aqr_2024` / `streaky_returns` / `hidden_value_of_streaky`) and rewrites the same surfaces with a Sharpe-ratio-paper narrative, including a "Sharpe-ratio Δ" tag and the published t-stats in the verdict strip's claim column. Symmetrically, the engine's "AQR sorts JKP factors not individuals" data-quality flag is gated to actually-AQR runs via `spec.paper_title`, and `_rewrite_aqr_flag_for_non_aqr_paper(result, paper_id)` in `app/main.py` strips the misleading flag from any cached `BacktestResult` returned for Lo-MacKinlay / Poterba-Summers / other VR-on-stocks papers and substitutes a "VR strategy ran on individual stocks — concept-implied portfolio" note.

The **known-paper headline-claim registry** (`_known_headline_claim` in `app/main.py`) is the manual override path for papers whose headline doesn't fit `monthly_long_short_return` cleanly enough for A1 to extract verbatim. Two papers ship with hardcoded entries today: Cheng-Hameed-Subrahmanyam-Titman 2017 (1.683%/mo, t=7.80 — the 1M reversal within the 3M-loser quintile, a nested conditioning sort A1 can't express) and Asness-Moskowitz-Pedersen 2013 (5.4%/yr ⇒ 0.0045/mo, t=2.08 — US-stock momentum P3-P1 from Table I Panel A, which the PDF stores in mirrored character order). The override fires inside `_run_pipeline` after A1, before D2/D3 see the spec, so the verdict strip / D2 mutation chain / D3 gap-attribution all compare against the right number. JT 1993 isn't in the registry because A1 extracts the headline correctly from the abstract (0.0095/mo, t=3.07).
